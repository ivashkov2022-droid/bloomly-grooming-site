...
function enhanceSemanticMarkup(source: string) {
  let html = source;
...
const optimizedPage = enhanceSemanticMarkup(bloomlyPage)
...
<script type="application/ld+json">${JSON.stringify({
  ...
  .replace(
    '</body>',
    `<script src="js/bloomly-optimizations.js?v=${assetVersion}" defer></script></body>`,
  );
